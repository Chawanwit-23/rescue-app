    ...
    <div className="min-h-screen bg-slate-100 flex flex-col items-center py-6 px-4 font-sans overflow-y-auto">
      <div className="w-full max-w-md bg-white rounded-3xl shadow-xl overflow-hidden border border-slate-100 relative mb-6">
        <div className="bg-slate-900 p-6 text-white text-center relative overflow-hidden">
          ...
          <Link
            to="/login"
            className="absolute top-4 right-4 flex items-center gap-1.5 bg-white/10 hover:bg-white/20 backdrop-blur-md text-white px-3 py-1.5 rounded-full text-[10px] font-bold transition-all border border-white/10 z-50 cursor-pointer hover:scale-105 active:scale-95"
          >
          ...
          <Link
            to="/evacuation"
            className="absolute top-4 left-4 flex items-center gap-1.5 bg-white/10 hover:bg-white/20 backdrop-blur-md text-white px-3 py-1.5 rounded-full text-[10px] font-bold transition-all border border-white/10 z-50 cursor-pointer"
          >
          ...
          <div className="relative z-10 flex flex-col items-center">
            ...
            <p className="text-slate-400 text-xs mt-1">
              ระบบ AI กู้ภัยอัจฉริยะ (Flood Rescue)
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-5">
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-end px-1">
              ...
              <button
                ...
                className="text-[10px] bg-blue-50 text-blue-600 border border-blue-100 px-3 py-1.5 rounded-full hover:bg-blue-100 flex items-center gap-1 font-bold transition-colors"
              >
            ...
            <div className="flex justify-between items-center px-2 text-[10px] text-slate-400">
              ...
              {isResolvingAddress && (
                <span className="flex items-center gap-1 text-orange-500 font-bold">
                  <Loader2 size={10} className="animate-spin" /> ดึงชื่อถนน...
                </span>
              )}
            </div>
          </div>
          ...
          <div className="space-y-3 bg-slate-50 p-4 rounded-2xl border border-slate-200/60">
            <label className="font-bold text-slate-700 text-sm flex items-center gap-2">
              <Home size={18} className="text-orange-500" /> 2. ที่อยู่
              (แก้ไขได้)
            </label>
            <p className="text-[10px] text-slate-400 ml-6 -mt-2 mb-2">
              *กรุณากรอกตำบลและอำเภอด้วยตนเอง
            </p>

            <div className="relative">
              <label className="text-[10px] text-slate-500 font-semibold ml-1 mb-0.5 block">
          ...
          <div className="space-y-4 bg-blue-50 p-4 rounded-2xl border border-blue-100/60">
            ...
            <div className="flex items-center justify-between bg-white p-2 rounded-xl border border-blue-100">
              <label className="text-xs font-bold text-slate-600 flex items-center gap-2 pl-2">
                <Users size={16} className="text-blue-400" /> ผู้ประสบภัย (คน)
              </label>
              <div className="flex items-center gap-1">
                <button
                  type="button"
                  onClick={() => setPeopleCount(Math.max(1, peopleCount - 1))}
                  className="w-8 h-8 flex items-center justify-center bg-blue-50 text-blue-600 hover:bg-blue-100 rounded-lg transition font-bold"
                >
                  -
                </button>
                <span className="w-10 text-center font-bold text-lg text-slate-700">
                  {peopleCount}
                </span>
                <button
                  type="button"
                  onClick={() => setPeopleCount(peopleCount + 1)}
                  className="w-8 h-8 flex items-center justify-center bg-blue-600 text-white hover:bg-blue-700 rounded-lg transition font-bold shadow-sm shadow-blue-200"
                >
                  +
                </button>
              </div>
            ...
            <div>
              <label className="text-[10px] font-bold text-slate-500 ml-1 mb-1 block">
                ระดับน้ำปัจจุบัน
              </label>
              <div className="relative">
                ...
                <select
                  ...
                  className="w-full pl-9 p-2.5 bg-white border border-blue-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500 appearance-none"
                >
                  ...
                  <option value="ท่วมถึงอก/มิดหัว">
                    ท่วมถึงอก / มิดหัว (วิกฤต)
                  </option>
            ...
            <div>
              ...
              <div className="flex gap-2">
                {["ผู้ประสบภัยเอง", "ญาติ/คนรู้จัก", "พลเมืองดี"].map(
                  (type) => (
                    <button
                      key={type}
                      type="button"
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <div className="relative mt-1">
                  ...
                  <input
                    name="name"
                    className="w-full pl-9 p-2.5 bg-white border border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-slate-400"
                    required
            ...
            <div>
              ...
              <div className="relative mt-1">
                ...
                <textarea
                  name="description"
                  className="w-full pl-9 p-3 bg-white border border-slate-200 rounded-xl text-sm h-20 outline-none focus:ring-2 focus:ring-slate-400 resize-none"
                  placeholder="เช่น ผู้ป่วยติดเตียง, คนชรา, อาหารหมด, ตัดไฟแล้ว..."
                />
              </div>
            </div>
          </div>
          ...
          {/* 5. Photo */}
          <div
            className={`border-2 border-dashed rounded-2xl p-6 text-center cursor-pointer relative group transition-all ${
              imageBase64
          ...
          >
            ...
            {imageBase64 ? (
              <div className="relative">
                <img
                  src={imageBase64}
                  className="h-40 mx-auto rounded-lg shadow-md object-cover"
                />
                <div className="absolute bottom-2 right-1/2 translate-x-1/2 bg-black/60 text-white text-[10px] px-2 py-1 rounded-full backdrop-blur-sm">
                  แตะเพื่อเปลี่ยนรูป
                </div>
              </div>
            ) : (
              <div className="py-2">
                ...
                <h3 className="text-sm font-bold text-slate-700">
                  ถ่ายรูปหน้างาน (จำเป็น)
                </h3>
                <p className="text-xs text-slate-400 mt-1">
                  เพื่อให้ AI ประเมินความรุนแรง
                </p>
              </div>
            )}
          </div>
          ...
          <button
            disabled={loading}
            className="w-full py-4 bg-gradient-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 text-white font-bold text-lg rounded-2xl shadow-lg shadow-red-200 flex justify-center items-center gap-2 transition-all active:scale-95 disabled:opacity-70 disabled:cursor-not-allowed"
          >
            {loading ? (
              <Loader2 className="animate-spin" size={24} />
            ) : (
              <>
                <Send size={20} /> ส่งแจ้งเหตุทันที
              </>
            )}
          </button>
        </form>
      ...
      <footer className="text-center text-slate-400 text-[10px] font-medium opacity-80 pb-6">
        ...
        <p className="flex items-center justify-center gap-1 mt-1">
          ...
          />{" "}
          by <span className="text-slate-600 font-bold">Chawanwit</span>
        </p>
      </footer>
    </div>
  );
