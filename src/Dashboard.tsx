  ...
  const html = `
    ...
    ">
      <div style="display:flex; align-items:center; justify-content:center; gap:4px;">
        ...
        ${
          status === "waiting"
            ? `<span style="background:${borderColor}; color:white; border-radius:50%; width:16px; height:16px; font-size:10px; display:flex; align-items:center; justify-content:center;">${score}</span>`
            : ""
        }
      </div>
      <div style="
...
export default function Dashboard() {
  ...
  const formatTime = (timestamp: any) => {
    if (!timestamp) return "";
    return new Date(timestamp.seconds * 1000).toLocaleTimeString("th-TH", {
      hour: "2-digit",
      minute: "2-digit",
    });
  };
      ...
      <div className="w-full h-[50vh] md:w-1/3 md:h-full md:min-w-[400px] bg-white shadow-2xl z-20 flex flex-col border-b md:border-b-0 md:border-r border-gray-200">
        ...
        <div className="p-4 md:p-5 bg-slate-900 text-white shadow-md flex-shrink-0">
          <div className="flex justify-between items-center mb-3">
            ...
            <Link
              to="/"
              className="text-[10px] md:text-xs bg-slate-800 px-3 py-1.5 rounded-full text-slate-400 border border-slate-700 hover:bg-slate-700 transition"
            >
              ← แจ้งเหตุ
            </Link>
          </div>
          ...
          <div className="flex flex-col gap-2 bg-slate-800 p-2 md:p-3 rounded-lg border border-slate-700">
            ...
            <div className="flex gap-2">
              <div className="flex-1">
                <select
                  ...
                  className="w-full bg-slate-700 text-[10px] md:text-xs text-white border border-slate-600 rounded-md p-1.5 outline-none"
                >
                  {provinces.map((p) => (
                    <option key={p} value={p}>
                      {p === "ทั้งหมด" ? "ทุกจังหวัด" : p}
                    </option>
                  ))}
                </select>
              </div>
            ...
            {/* แถวที่ 2: ตำบล (เพิ่มกลับมาแล้วครับ!) */}
            <div>
              <select
        ...
        <div className="p-2 md:p-3 border-b border-gray-100 bg-white sticky top-0 z-10 flex-shrink-0">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-1.5 md:py-2 text-sm border border-gray-200 rounded-lg focus:ring-blue-500 outline-none bg-gray-50"
            />
          </div>
        </div>
              ...
              >
                <div className="flex justify-between items-start gap-2">
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-gray-500 line-clamp-1 mt-1 border-l-2 border-slate-200 pl-1">
                      "{req.description}"
                    </p>
                    ...
                    <div className="flex gap-2 mt-2">
                      <button
                        onClick={(e) =>
                          openGoogleMaps(req.location.lat, req.location.lng, e)
                        }
                        className="px-2 py-1 bg-blue-50 text-blue-600 rounded hover:bg-blue-100 text-[10px] flex items-center gap-1 border border-blue-200"
                      >
                        <Navigation size={10} /> นำทาง
                      </button>
                      {req.status === "waiting" && (
                        <button
                          onClick={(e) => updateStatus(req.id, "inprogress", e)}
                          className="px-2 py-1 bg-orange-50 text-orange-600 rounded hover:bg-orange-100 text-[10px] flex items-center gap-1 border border-orange-200"
                        >
                          รับงาน
                        </button>
                      )}
                      {req.status === "inprogress" && (
                        <button
                          onClick={(e) => updateStatus(req.id, "completed", e)}
                          className="px-2 py-1 bg-green-50 text-green-600 rounded hover:bg-green-100 text-[10px] flex items-center gap-1 border border-green-200"
                        >
                          ปิดเคส
                        </button>
                      )}
                  ...
                  {req.imageUrl && (
                    <img
                      src={req.imageUrl}
                      className="w-12 h-12 md:w-16 md:h-16 rounded-lg object-cover border bg-white"
                    />
                  )}
                </div>
              </div>
