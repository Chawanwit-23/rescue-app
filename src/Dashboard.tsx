...
const { 
  ...
  UserCheck, X, Skull, Trash2, LogOut, Tent 
} = LucideIcons as any;
...
// --- 3. Marker Logic (Fixed Position & Design) ---
const createLabelIcon = (name: string, score: number, status: string, isBlackCase?: boolean) => {
  ...
  const html = `
    <div style="
      ...
      /* จัดตำแหน่งให้ปลายหมุดจิ้มที่พิกัดพอดี */
      position: absolute; left: 0px; top: 0px; transform: translate(-50%, calc(-100% - 10px));
      min-width:80px;
      ${status === 'completed' && !isBlackCase ? 'filter: grayscale(100%);' : ''}
      ${isBlackCase ? 'border-width: 3px; animation: pulse-black 2s infinite;' : ''}
    ">
    ...
    <style>
      @keyframes pulse-black { 0% { box-shadow: 0 0 0 0 rgba(0, 0, 0, 0.7); } 70% { box-shadow: 0 0 0 10px rgba(0, 0, 0, 0); } 100% { box-shadow: 0 0 0 0 rgba(0, 0, 0, 0); } }
    </style>
  `;
  
  return L.divIcon({ className: "custom-div-icon", html: html, iconSize: [0, 0], iconAnchor: [0, 0] });
};
...
function MapFlyTo({ location }: { location: [number, number] | null }) {
  const map = useMap();
  useEffect(() => { 
    if (location) map.flyTo(location, 16, { duration: 1.5, easeLinearity: 0.25 }); 
  }, [location, map]);
  return null;
}
...
function StatCard({ label, count, color, icon }: any) {
  return (
    <div className={`
        ...
        min-w-[100px] md:min-w-0 flex-shrink-0 cursor-default
    `}>
...
export default function Dashboard() {
  ...
  const confirmAcceptWork = async (e: React.FormEvent) => {
    ...
    try {
        await updateDoc(doc(db, "requests", targetCaseId), { status: 'inprogress', rescuerName: officerForm.name, rescuerContact: officerForm.phone });
  ...
  const markAsBlackCase = async (id: string, e?: any) => {
    e?.stopPropagation();
    if(!confirm("💀 ยืนยันพบร่างผู้เสียชีวิต?\n\n(สถานะจะเปลี่ยนเป็น 'เคสดำ' ทันที)")) return;
    try { await updateDoc(doc(db, "requests", id), { isBlackCase: true }); } catch (err) { console.error(err); }
  };

  const finishBlackCase = async (id: string, e?: any) => {
    e?.stopPropagation();
    if(!confirm("⚰️ ยืนยันการเก็บกู้เสร็จสิ้น?\n\n(ข้อมูลจะถูกลบออกจากระบบ)")) return;
    try { await deleteDoc(doc(db, "requests", id)); } catch (err) { console.error(err); }
  };
  ...
  // 🟢 Click Item -> Fly to Map & Collapse Sheet
  const handleItemClick = (req: RequestData) => {
      if (req.location) {
          setSelectedLocation([req.location.lat, req.location.lng]);
          if (window.innerWidth < 768) setSheetHeight(12); // ย่อ Sheet ในมือถือเพื่อให้เห็นแมพ
      }
  };
    ...
    <div className="flex flex-col-reverse md:flex-row h-screen bg-slate-50 overflow-hidden font-sans text-slate-800 relative selection:bg-blue-100">
      ...
      {showAcceptModal && (
        <div className="fixed inset-0 z-[2000] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden scale-100 animate-in zoom-in-95 duration-200">
                <div className="bg-orange-500 p-4 text-white flex justify-between items-center"><h3 className="font-bold text-lg flex items-center gap-2"><UserCheck size={20}/> ลงทะเบียนรับงาน</h3><button onClick={() => setShowAcceptModal(false)} className="text-white/80 hover:text-white"><X size={20}/></button></div>
                <form onSubmit={confirmAcceptWork} className="p-5 space-y-4"><div className="text-sm text-slate-500 bg-orange-50 p-3 rounded-lg border border-orange-100">ระบุชื่อ/เบอร์ เพื่อการประสานงาน</div><input autoFocus type="text" className="w-full p-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-orange-500 outline-none" placeholder="ชื่อเจ้าหน้าที่ / หน่วยงาน" value={officerForm.name} onChange={e => setOfficerForm({...officerForm, name: e.target.value})}/><input type="tel" className="w-full p-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-orange-500 outline-none" placeholder="เบอร์ติดต่อ" value={officerForm.phone} onChange={e => setOfficerForm({...officerForm, phone: e.target.value})}/><div className="flex gap-2 pt-2"><button type="button" onClick={() => setShowAcceptModal(false)} className="flex-1 py-2.5 bg-slate-100 text-slate-600 rounded-xl font-bold text-sm hover:bg-slate-200">ยกเลิก</button><button type="submit" className="flex-1 py-2.5 bg-orange-500 text-white rounded-xl font-bold text-sm hover:bg-orange-600 shadow-lg shadow-orange-200">ยืนยัน</button></div></form>
            </div>
        </div>
      )}

      {/* Sidebar */}
      <div className={`w-full md:w-[450px] bg-white shadow-[0_-5px_30px_rgba(0,0,0,0.1)] z-[1000] flex flex-col border-r border-slate-200 absolute bottom-0 left-0 md:relative md:h-full rounded-t-[2rem] md:rounded-none overflow-hidden ${!isDragging ? 'transition-[height] duration-500 cubic-bezier(0.32, 0.72, 0, 1)' : ''}`} style={{ height: `${window.innerWidth < 768 ? sheetHeight : 100}%` }}>
        <div className="md:hidden w-full h-[32px] bg-white flex justify-center items-center cursor-grab active:cursor-grabbing border-b border-slate-50 flex-shrink-0 touch-none" onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}><div className="w-10 h-1 bg-slate-200 rounded-full"></div></div>
        ...
        <div className="p-4 md:p-6 bg-slate-900 text-white shadow-xl relative overflow-hidden flex-shrink-0">
            ...
            <div className={`relative z-10 flex justify-between items-center mb-4 ${sheetHeight < 20 ? 'hidden md:flex' : ''}`}>
              <h1 className="text-xl md:text-2xl font-black flex items-center gap-2 tracking-tight"><span className="bg-red-600 p-1.5 rounded-lg shadow-red-900/50 shadow-lg"><Siren size={18} className="text-white animate-pulse" /></span> WAR ROOM</h1>
              <div className="flex items-center gap-2">
                  <Link to="/evacuation" className="p-1.5 bg-white/10 rounded-full hover:bg-white/20 transition" title="จุดอพยพ"><Tent size={16}/></Link>
                  ...
                  <button onClick={handleLogout} className="p-1.5 bg-red-500 text-white rounded-full hover:bg-red-600 shadow-md transition" title="ออกจากระบบ"><LogOut size={14} /></button>
              </div>
            </div>

            <div className={`flex flex-row overflow-x-auto gap-3 pb-2 -mx-1 px-1 no-scrollbar md:grid md:grid-cols-3 lg:grid-cols-2 xl:grid-cols-3 md:overflow-visible md:pb-0 md:gap-2 transition-all duration-300 ${sheetHeight < 20 ? 'opacity-0 translate-y-4 md:opacity-100 md:translate-y-0 pointer-events-none' : 'opacity-100 translate-y-0'}`}>
        ...
        <div className={`flex-1 flex flex-col overflow-hidden transition-opacity duration-200 ${sheetHeight < 20 ? 'opacity-0 md:opacity-100 pointer-events-none' : 'opacity-100'}`}>
            ...
            <div className="flex-1 overflow-y-auto p-3 md:p-4 space-y-3 bg-slate-50/50 pb-20 md:pb-4">
              {filteredRequests.map((req) => {
                ...
                let cardBorder = isDone ? (req.isBlackCase ? "border-l-black border-l-[6px] bg-slate-100" : "border-l-slate-300") : (score >= 8 ? "border-l-red-500" : (score >= 5 ? "border-l-orange-400" : "border-l-emerald-500"));
                let badgeStyle = isDone ? (req.isBlackCase ? "bg-black text-white shadow-md shadow-black/30" : "bg-slate-100 text-slate-500") : (score >= 8 ? "bg-red-50 text-red-600" : (score >= 5 ? "bg-orange-50 text-orange-600" : "bg-emerald-50 text-emerald-600"));
                
                return (
                  <div key={req.id} onClick={() => handleItemClick(req)} className={`bg-white rounded-xl p-3 shadow-sm hover:shadow-lg hover:-translate-y-0.5 border border-slate-100 ${cardBorder} border-l-[4px] transition-all cursor-pointer group`}>
                    <div className="flex justify-between items-start mb-2"><div className="flex items-center gap-2"><span className={`text-[10px] font-bold px-2 py-0.5 rounded-md uppercase tracking-wide flex items-center gap-1 ${badgeStyle}`}>{req.isBlackCase ? <Skull size={10} className="text-red-500"/> : (isDone ? <CheckCircle2 size={10}/> : <Activity size={10}/>)} {req.isBlackCase ? "BLACK CASE" : (isDone ? "DONE" : `RISK ${score}`)}</span></div><span className="text-[10px] text-slate-400 flex items-center gap-1 bg-slate-50 px-2 py-0.5 rounded-full font-medium"><Clock size={10}/> {new Date(req.timestamp?.seconds * 1000).toLocaleTimeString("th-TH",{hour:'2-digit',minute:'2-digit'})}</span></div>
                    <div className="flex gap-3">
                      <div className="flex-1">
                          <h3 className="font-bold text-slate-800 text-sm md:text-base line-clamp-1">{req.name}</h3>
                          <div className="flex items-center gap-2 text-[10px] md:text-xs text-slate-500 mt-0.5 mb-2 font-medium"><span className="flex items-center gap-0.5"><Phone size={10}/> {req.contact}</span><span className="w-1 h-1 rounded-full bg-slate-300"></span><span className="flex items-center gap-0.5 text-blue-500 bg-blue-50 px-1.5 rounded"><Users size={10}/> {req.peopleCount}</span></div>
                          ...
                          {req.address && <div className="flex items-start gap-2 text-[10px] text-slate-500 mb-3 bg-slate-50 p-2 rounded border border-slate-100"><MapPin size={12} className="mt-0.5 text-red-500 flex-shrink-0" /><div><span className="font-bold text-slate-700 block">{req.address.details}</span><span>{req.address.subdistrict} {req.address.district} {req.address.province}</span></div></div>}
                          {(req.status === 'inprogress' || req.status === 'completed') && req.rescuerName && (<div className="flex items-center gap-2 mb-2 bg-indigo-50 text-indigo-700 px-2 py-1 rounded-md text-[10px] border border-indigo-100"><UserCheck size={12}/> <span>ดูแลโดย: <b>{req.rescuerName}</b></span></div>)}
      ...
      <div className="w-full h-full relative z-0 bg-slate-200">
        <MapContainer center={[13.7563, 100.5018]} zoom={10} style={{ height: "100%", width: "100%" }} zoomControl={false}>
          {/* 🟢 Google Maps Roadmap (Roads visible) */}
          ...
          {filteredRequests.map((req) => {
             ...
             const score = req.ai_analysis?.risk_score || 0;
             return (
               <Marker key={req.id} position={req.location} icon={createLabelIcon(req.name, score, req.status, req.isBlackCase)} zIndexOffset={req.status === 'completed' ? -1000 : (req.isBlackCase ? 2000 : 100)}>
                  <Popup>
                     <div className="flex flex-col font-sans">
                        {req.imageUrl ? (<div className="h-24 w-full bg-cover bg-center rounded-t-lg relative" style={{backgroundImage: `url(${req.imageUrl})`}}><div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent"></div><span className={`absolute bottom-2 left-2 text-[10px] font-bold px-2 py-0.5 rounded text-white ${req.isBlackCase ? 'bg-black' : (req.status==='waiting'?'bg-red-600':req.status==='inprogress'?'bg-orange-500':'bg-emerald-600')}`}>{req.isBlackCase ? 'URGENT: RECOVERY' : (req.status==='waiting' ? `RISK ${score}` : req.status==='inprogress' ? 'WORKING' : 'DONE')}</span></div>) : (<div className="h-10 w-full bg-slate-100 flex items-center justify-center border-b border-slate-200"><span className="text-xs text-slate-400 font-medium">ไม่มีรูปภาพ</span></div>)}
                        <div className="p-3">
                           <h3 className="font-bold text-slate-800 text-sm mb-1 line-clamp-1">{req.name}</h3>
                           <div className="flex items-center gap-2 text-xs text-slate-500 mb-2"><span className="flex items-center gap-0.5"><Phone size={10}/> {req.contact}</span><span className="flex items-center gap-0.5 text-blue-600 bg-blue-50 px-1 rounded"><Users size={10}/> {req.peopleCount}</span></div>
                           ...
                           <div className="flex flex-col gap-2">
                              <div className="flex gap-2"><button onClick={(e) => openMaps(req.location!.lat, req.location!.lng, e)} className="flex-1 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-lg text-xs font-bold transition flex items-center justify-center gap-1"><Navigation size={12}/> ไป</button>{req.status === 'waiting' && <button onClick={(e) => initiateAccept(req.id, e)} className="flex-1 py-1.5 text-white rounded-lg text-xs font-bold flex items-center justify-center gap-1 shadow-sm bg-orange-500 hover:bg-orange-600">รับงาน</button>}{req.status === 'inprogress' && !req.isBlackCase && <button onClick={(e) => closeCase(req.id, e)} className="flex-1 py-1.5 text-white rounded-lg text-xs font-bold flex items-center justify-center gap-1 shadow-sm bg-emerald-600 hover:bg-emerald-700">ปิดงาน</button>}{req.isBlackCase && <button onClick={(e) => finishBlackCase(req.id, e)} className="flex-1 py-1.5 text-white rounded-lg text-xs font-bold flex items-center justify-center gap-1 shadow-sm bg-black hover:bg-gray-800"><Trash2 size={12}/> ยืนยันเก็บกู้</button>}</div>
                              {req.status === 'inprogress' && !req.isBlackCase && <button onClick={(e) => markAsBlackCase(req.id, e)} className="w-full py-1.5 text-white rounded-lg text-xs font-bold flex items-center justify-center gap-1 shadow-sm bg-slate-700 hover:bg-black"><Skull size={12}/> แจ้งเคสดำ</button>}
                           </div>
                        </div>
                     </div>
                  </Popup>
